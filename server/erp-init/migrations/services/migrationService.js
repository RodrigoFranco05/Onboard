...
function createMigrationRunner(sequelize, tenantName) {
  // Leer archivos de migración manualmente para evitar problemas con glob en Windows
  ...
  try {
    migrationFiles = fs.readdirSync(migrationsDir)
      ...
      .sort(); // Ordenar por nombre para mantener orden cronológico
  } catch (error) {
    console.warn(`⚠️  No se pudo leer directorio de migraciones: ${migrationsDir}`);
    migrationFiles = [];
  }
  
  // Crear configuración explícita de migraciones
  const migrations = migrationFiles.map(file => {
    ...
    return {
      ...
      path: fullPath,
      up: async () => {
        console.log(`🔄 [${tenantName}] Aplicando: ${file}`);
        ...
        try {
          ...
          console.log(`✅ [${tenantName}] Completada: ${file} (${duration}ms)`);
        } catch (error) {
          console.error(`❌ [${tenantName}] Error en: ${file}`);
      ...
      down: async () => {
        console.log(`🔄 [${tenantName}] Revirtiendo: ${file}`);
        try {
          const migration = require(fullPath);
          await migration.down(sequelize.getQueryInterface(), sequelize.constructor);
          console.log(`✅ [${tenantName}] Revertida: ${file}`);
        } catch (error) {
          console.error(`❌ [${tenantName}] Error revirtiendo: ${file}`);
          console.error(`   💥 ${error.message || error}`);
          if (error.sql) {
            console.error(`   📝 SQL: ${error.sql}`);
          }
