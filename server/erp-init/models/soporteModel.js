...
function soporteModelInit(sequelize) {
  ...
  // ============================================
  // MODELO: AccionCrossTenant
  // Snapshot del actor real cuando una acción se ejecuta cross-tenant.
  // La fila se inserta solo si el writer estaba operando desde otro tenant
  // (vía dropdown del main support). El "actor" en la fila de ticket/mensaje/
  // evento del tenant destino es el usuario lutenteERP local, y acá guardamos
  // quién fue realmente.
  // ============================================
  const Soporte_AccionCrossTenant = sequelize.define('Soporte_AccionCrossTenant', {
    id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true },
    accion: {
      type: DataTypes.STRING(50),
      allowNull: false,
      comment: "Ej. 'create_ticket', 'add_message', 'take', 'reassign', 'change_state', 'change_priority', 'change_category', 'edit_description', 'delete_ticket'"
    },
    entidadTipo: {
      type: DataTypes.STRING(30),
      allowNull: false,
      comment: "'Ticket' | 'TicketMensaje' | 'TicketEvento'"
    },
    entidadId: {
      type: DataTypes.INTEGER,
      allowNull: false
    ...
    // Target del acto cuando la acción involucra otro usuario (assign/reassign/take).
    // Snapshot del usuario assignee real del tenant central. En la BD destino el
    // agentId del ticket apunta a lutenteERP (máscara); el assignee real vive acá.
    targetTenantOrigen: {
      type: DataTypes.STRING(100),
      allowNull: true
    },
    ...
    targetApellidoOrigen: {
      type: DataTypes.STRING(100),
      allowNull: true
    }
  }, {
    tableName: 'soporte_AccionCrossTenant',
    timestamps: true,
    indexes: [
      { fields: ['entidadTipo', 'entidadId'] },
      { fields: ['actorTenantOrigen', 'actorIdOrigen'] },
      { fields: ['targetTenantOrigen', 'targetIdOrigen'] }
    ]
  });
